...
public class SSS extends Calculation {
        ...
        try (FileInputStream fis = new FileInputStream(XLSX_FILE_PATH);
            ...
            for (int rowIndex = 1; rowIndex <= sheet.getLastRowNum(); rowIndex++) {
                ...
                if (row != null) {
                    ...
                    double contribution = parseCellValueAsDouble(row.getCell(1)); // Contribution amount
    ...
    private static double[] parseSssCompensationRange(String compensationRange) {
        ...
        // Handle the "Above X" format
        if (compensationRange.startsWith("Above")) {
            double start = parseNumber(compensationRange.replace("Above", "").trim());
